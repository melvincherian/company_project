...
class _ChangeIndustryScreenState extends State<BusinessIndustryScreen> {
  ...
  child: ElevatedButton(
    onPressed: _selectedIndexes.isEmpty
        ? null
        : () async {
            // Call fetchPosters before navigating
            final homeProvider = Provider.of<HomeProvider>(context, listen: false);
            await homeProvider.fetchPosters();

            final selectedCategories = _selectedIndexes
                .map((index) => _filteredCategories[index])
                .toList();

            Navigator.push(
              context,
              MaterialPageRoute(
                builder: (context) => NavbarScreen(selectedCategories: selectedCategories),
              ),
            );
          },
    style: ElevatedButton.styleFrom(
      backgroundColor: const Color(0xFF413B99),
